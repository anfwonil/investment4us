...
def tab_market():
    ...
    if not valid_cols:
        ...
        st.session_state["m_ycols"] = valid_cols  # 선택 복구
    ycols = valid_cols
    ...
    # 3) dtype 강제 (객체 섞이면 선이 안 그려질 수 있음)
    for c in ycols:
        plot_df[c] = pd.to_numeric(plot_df[c], errors="coerce")
    ...
    fig.update_layout(uirevision="mkt", xaxis_rangeslider_visible=False)
    st.plotly_chart(
        fig,
        use_container_width=True,
...
# -------------------- 한 줄 검색바 (Streamlit columns로 가로 배치) --------------------
# 기존의 "검색 플로팅 UI" 블록을 아래 코드로 통째로 교체

st.markdown("""
<style>
  /* 검색바/버튼 높이 맞춤 + 아래 여백 최소화 */
