...
def compute_price_indicators(df_long: pd.DataFrame, ma_windows=(20,60,120), vol_window=60):
    ...
    data = df_long.groupby("Ticker", group_keys=False).apply(f)
    return {"data": data}
...
def tab_research_global():
    ...
    base_long = pd.concat([yf_long, fd_long], ignore_index=True)
    ...
    with tA:
        ...
        sel = st.multiselect("표시 자산(비교는 Base=100)", options=all_tickers, default=all_tickers[:min(4,len(all_tickers))], key="rgA_sel")
        if sel:
            disp = data[data["Ticker"].isin(sel)].copy()
            # Base=100 리베이스
            def to_base100(g):
                g = g.sort_values("Date").copy()
                first = g["Close"].dropna().iloc[0]
                g["Base100"] = (g["Close"]/first)*100.0 if pd.notna(first) else pd.NA
                return g
            disp = disp.groupby("Ticker", group_keys=False).apply(to_base100)
            ...
            snap = (disp.sort_values("Date").groupby("Ticker").tail(1))[["Ticker"]+cols]
            for c in cols:
                if c in snap: snap[c] = (snap[c]*100).map(lambda x: f"{x:+.2f}%" if pd.notna(x) else "")
            st.markdown("**기간별 수익률 스냅샷**")
            st.dataframe(snap.set_index("Ticker"))
            ...
            for w in (20,60,120):
                col = f"SMA{w}"
                if col in one_df:
                    fig_ma.add_trace(go.Scatter(x=one_df["Date"], y=one_df[col], mode="lines",
                                                name=f"SMA{w}", line=dict(dash="dot")))
    ...
    with tD:
        ...
        ret = base_long.pivot(index="Date", columns="Ticker", values="Close").pct_change().dropna()
        if not ret.empty:
...
# ==================== MAIN ====================
tab1, tab2, tab3 = st.tabs(["시장/티커", "포트폴리오", "가격 리서치"])
with tab1:
    tab_market()
with tab2:
    tab_portfolio()
